...
DB_PATH="${GCARD_IMDB_DB_PATH:-${PROJECT_ROOT}/experiment/run_tmp/gcard_master_k2_arm1_d5_current_catalogs_retest/dbs/imdb}"
GRAPH="${GCARD_IMDB_GRAPH:-imdb_k2_arm1_d5_after_offset_fix}"
K="${1:-2}"
SAMPLE_SIZE="${2:-500}"
MAX_GRAPH="${3:-5}"
OUT_DIR="${GCARD_OUT_DIR:-${PROJECT_ROOT}/experiment/result/gcard_query}"
PATTERN_DIR="${PROJECT_ROOT}/experiment/patterns/gcard/imdb"

mkdir -p "$OUT_DIR"
out="$OUT_DIR/imdb_k${K}_ss${SAMPLE_SIZE}.csv"
tmp="$(mktemp)"
trap 'rm -f "$tmp"' EXIT
