#!/usr/bin/env bash
#
# Benchmark: measure how random updates degrade GCard estimation accuracy.
#
# Flow:
#   1. Load LDBC data via load_ldbc (single minigu process)
#   2. Run baseline queries (round 0)
#   3. For each update round:
#        random_update(ratio, roundX/graph.bin)
...
#
# Usage:
#   ./bench_update_impact.sh [OPTIONS] <SF>
#
# Options:
#   --max-k K           Maximum k for gcard_query (default: 3)
#   --sample-size N     Sample size (default: 500)
#   --max-graph N       Max spanning trees (default: 5)
#   --ratios R1,R2,...  Comma-separated update ratios (default: 0.05,0.05,0.05,0.05,0.05)
#   --seed S            Base random seed (default: 42)
#
# Example:
#   ./bench_update_impact.sh sf1
#   ./bench_update_impact.sh --ratios 0.05,0.10,0.10 sf0.1
...
SAMPLE_SIZE=500
MAX_GRAPH=5
RATIOS="0.05,0.05,0.05,0.05,0.05"
BASE_SEED=42
SF=""
...
RESULT_DIR="$PROJECT_DIR/experiment/result/update_impact/${SF}"
mkdir -p "$RESULT_DIR"
...
SCRIPT_FILE="$TMP_DIR/bench_update.gql"
...
# Rounds 1..N: update + export snapshot + query
for i in $(seq 0 $((NUM_ROUNDS - 1))); do
    ...
    mkdir -p "$round_dir"
    echo ":time call random_update(${ratio}, \"${graph_bin}\", ${seed})" >> "$SCRIPT_FILE"
...
log "Running minigu with ${NUM_ROUNDS} update rounds + queries..."
...
# Parse all :time outputs in execution order.
TIME_LINES=()
while IFS= read -r line; do
    TIME_LINES+=("$line")
...
for i in $(seq 0 $((NUM_ROUNDS - 1))); do
    round=$((i + 1))
    ratio="${RATIO_ARR[$i]}"
    echo "${round},random_update,ratio=${ratio},$(next_time)" >> "$TIMING_CSV"
...
FINAL_CSV="${RESULT_DIR}/update_impact_results.csv"
...
# ── Phase 5: extract internal procedure timings from minigu logs ─────────────
...
awk '
BEGIN { round = 0 }
/\[random_update\] phase1 sample_targets:/ {
  sub(/^.*phase1 sample_targets: /, "", $0); sub(/s.*$/, "", $0);
  print "random_update," round ",phase1_sample_targets," $0;
}
/\[random_update\] phase2 append_log:/ {
  sub(/^.*phase2 append_log: /, "", $0); sub(/s.*$/, "", $0);
  print "random_update," round ",phase2_append_log," $0;
}
/\[random_update\] phase3 compact_log:/ {
  sub(/^.*phase3 compact_log: /, "", $0); sub(/s.*$/, "", $0);
  print "random_update," round ",phase3_compact_log," $0;
}
...
/\[random_update\] phase4a apply_pending:/ {
  sub(/^.*phase4a apply_pending: /, "", $0); sub(/s.*$/, "", $0);
  print "random_update," round ",phase4a_apply_pending," $0;
}
...
/\[random_update\] phase4b rebuild_dsgc:/ {
  sub(/^.*phase4b rebuild_dsgc: /, "", $0); sub(/s.*$/, "", $0);
  print "random_update," round ",phase4b_rebuild_dsgc," $0;
}
/\\[dsgc\\] encode_dirty:/ {
...
/\[random_update\] phase4 total:/ {
  sub(/^.*phase4 total: /, "", $0); sub(/s.*$/, "", $0);
  print "random_update," round ",phase4_total," $0;
}
/\[random_update\] total:/ {
  sub(/^.*total: /, "", $0); sub(/s.*$/, "", $0);
  round += 1;
  print "random_update," round ",total," $0;
}
