...
TMP_ROOT = EXP / "run_tmp/aids_merged_predicates_gcard"
DB = TMP_ROOT / "db"
...
def run_queries(args):
    ...
    if rows:
        ...
        pattern_paths = sorted(PATTERNS.glob("*/*.json"), key=pattern_key)
        rows = [{"topology": p.parent.name, "query": p.stem, "pattern": f"{p.parent.name}/{p.stem}", "true_cardinality": ""} for p in pattern_paths]

    gql = TMP_ROOT / "query.gql"
    log = RUN_ROOT / "aids_merged_predicates.log"
    lines = [
    ...
    with detail.open("w", newline="", encoding="utf-8") as f:
        ...
        for i, (row, est) in enumerate(zip(rows, estimates)):
            ...
            qe = qerror(est, true) if true not in (None, "") else ""
            w.writerow({
                "dataset": "aids_merged_predicates",
                "topology": row["topology"],
                "query": row["query"],
                "pattern": row["pattern"],
                "estimate": f"{est:.17g}",
                "true": true,
                "qerror": f"{qe:.17g}" if isinstance(qe, float) and math.isfinite(qe) else qe,
