SELECT COUNT(*)
FROM (SELECT * FROM person_likes_comment e1 WHERE e1.creationdate >= 1262304000000 AND e1.deletiondate >= 1356998400000) e1
...
JOIN (SELECT * FROM person v3 WHERE v3.gender = 'male') v3 ON v3.id = e2.dst
JOIN (SELECT * FROM comment v2 WHERE v2.explicitlydeleted = false) v2 ON v2.id = e1.dst
