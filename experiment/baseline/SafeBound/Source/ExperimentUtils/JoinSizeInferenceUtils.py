...
def evaluate_join_size_inference_pessimistic_cardinality_estimation(trueCardFile, 
    ...
    for i, queryJG in enumerate(queryJGs):
        queryHints = []
        hintFile = rootDirectory + 'pqo-opensource/output/results/' + benchmark + "_" + str(i) + "_SubQueryBounds.txt"
...
def evaluate_join_size_inference_neurocard(trueCardFile,  benchmark, queryJGs, outputFile):
    ...
    hintDF = pd.read_csv(rootDirectory + "Workloads/NeuroCardHintIndex_" + benchmark + ".csv", delimiter='|', names=["QueryLabel", "Tables"])
    estimatesDF = pd.read_csv(rootDirectory + "Data/Results/NeuroCard_Inference_" + benchmark + "SubQueries.csv")
