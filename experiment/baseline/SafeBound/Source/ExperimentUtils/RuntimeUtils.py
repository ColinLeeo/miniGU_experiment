...
def evaluate_runtime_pessimistic_cardinality_estimation(statsFile, benchmark, queryJGs,
    ...
    for j in range(runs):
        ...
        for i, queryJG in enumerate(queryJGs):
            ...
            runLabels.append(j)
            queryHints = []
            hintFile = rootFileDirectory + 'pqo-opensource/output/results/' + benchmark + "_" + str(i) + "_SubQueryBounds.txt"
            ...
            for hintString in open(hintFile, "r").readlines():
                ...
                queryHints.append(hint)
            inferenceTimeFile = rootFileDirectory + 'pqo-opensource/output/results/' + benchmark + "_" + str(i) + "_sketch_preprocessing.txt"
...
def evaluate_runtime_neurocard(benchmark, queryJGs, dbConn, outputFile, runs):
    ...
    hintDF = pd.read_csv(rootFileDirectory + "Workloads/NeuroCardHintIndex_" + benchmark + ".csv", delimiter='|', names=["QueryLabel", "Tables"])
    estimatesDF = pd.read_csv(rootFileDirectory + "Data/Results/NeuroCard_Inference_" + benchmark + "SubQueries.csv")
