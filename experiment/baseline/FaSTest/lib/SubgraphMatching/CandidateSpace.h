...
namespace SubgraphMatching {
    ...
    bool CandidateSpace::BuildCS(PatternGraph *query) {
        ...
        BuildInitialCS();
        stage_timer.Stop();
        fprintf(stderr, "[FaSTest-CS] BuildInitialCS %.2f ms\n", stage_timer.GetTime());
        fprintf(stderr, "[FaSTest-CS] InitCand");
        for (int i = 0; i < query_->GetNumVertices(); i++) {
            fprintf(stderr, " q%d(label=%d,deg=%d)=%zu", i, query_->GetVertexLabel(i), query_->GetDegree(i), candidate_set_[i].size());
        }
        fprintf(stderr, "\n");
        stage_timer = Timer();
        stage_timer.Start();
        RefineCS();
        stage_timer.Stop();
        fprintf(stderr, "[FaSTest-CS] RefineCS %.2f ms\n", stage_timer.GetTime());
        stage_timer = Timer();
        stage_timer.Start();
        ConstructCS();
        stage_timer.Stop();
        fprintf(stderr, "[FaSTest-CS] ConstructCS %.2f ms\n", stage_timer.GetTime());
        CStimer.Stop();
