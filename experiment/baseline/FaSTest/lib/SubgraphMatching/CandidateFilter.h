...
namespace GraphLib::SubgraphMatching {
    ...
    bool CandidateSpace::RefineCS(){
        ...
        int current_stage = 0;
        bool debug_refine = std::getenv("FASTEST_DEBUG_REFINE") != nullptr;
        if (debug_refine) {
            fprintf(stderr, "[FaSTest-CS] RefineStart qv=%d qe=%d max_queue=%d structure=%d neighbor=%d\n",
                    query_->GetNumVertices(), query_->GetNumEdges(), maximum_queue_cnt,
                    (int)opt.structure_filter, (int)opt.neighborhood_filter);
        }
        while (queue_pop_count < maximum_queue_cnt) {
            ...
            int removed_count = 0;
            if (debug_refine) {
                fprintf(stderr, "[FaSTest-CS] RefineIter stage=%d cur=q%d label=%d qdeg=%d cand=%d priority=%.4f queue=%d/%d\n",
                        current_stage, cur, query_->GetVertexLabel(cur), query_->GetDegree(cur), bef_cand_size,
                        cur_priority, queue_pop_count, maximum_queue_cnt);
            }
            ...
            for (int i = 0; i < candidate_set_[cur].size(); i++) {
                if (debug_refine && i > 0 && i % 10000 == 0) {
                    fprintf(stderr, "[FaSTest-CS] RefineProgress stage=%d cur=q%d checked=%d/%d removed=%d last_cand_degree=%d\n",
                            current_stage, cur, i, bef_cand_size, removed_count,
                            data_->GetDegree(candidate_set_[cur][i - 1]));
                }
                int cand = candidate_set_[cur][i];
                if (debug_refine && i < 5) {
                    fprintf(stderr, "[FaSTest-CS] RefineCandBegin stage=%d cur=q%d idx=%d cand=%d data_degree=%d query_edges=%zu data_edges=%zu\n",
                            current_stage, cur, i, cand, data_->GetDegree(cand),
                            query_->GetAllIncidentEdges(cur).size(), data_->GetAllIncidentEdges(cand).size());
                }
                bool valid = true;
                ...
                if (debug_refine && i < 5) {
                    fprintf(stderr, "[FaSTest-CS] RefineCandDone stage=%d cur=q%d idx=%d cand=%d valid=%d\n",
                            current_stage, cur, i, cand, valid);
                }
            ...
            if (debug_refine) {
                fprintf(stderr, "[FaSTest-CS] RefineIterDone stage=%d cur=q%d before=%d after=%d removed=%d\n",
                        current_stage, cur, bef_cand_size, aft_cand_size, removed_count);
            }
