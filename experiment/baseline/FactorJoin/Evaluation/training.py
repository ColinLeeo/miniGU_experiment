...
try:
    ...
    create_binned_cols = None
    get_query_binned_cards = None
...
def train_one_imdb(data_path, model_folder, n_dim_dist=1, bin_size=None, bucket_method="fixed_start_key",
    ...
    if prepare_sample:
        if create_binned_cols is None:
            raise ImportError("psycopg2 is required for prepare_sample")
    ...
    if materialize_sample and test_query_file is not None:
        if get_query_binned_cards is None:
            raise ImportError("psycopg2 is required for materialize_sample")
