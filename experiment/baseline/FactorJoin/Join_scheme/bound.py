...
class Bound_ensemble:
    ...
    def _factor_key_pdf(self, factor, key):
        if isinstance(factor.pdfs, dict):
            return factor.pdfs[key]
        axis = factor.variables.index(key)
        axes = tuple(i for i in range(factor.pdfs.ndim) if i != axis)
        if axes:
            return np.sum(factor.pdfs, axis=axes)
        return factor.pdfs

    def _factor_key_na(self, factor, key):
        if factor.na_values is None:
            return 1.0
        return factor.na_values.get(key, 1.0)
    ...
    def get_all_id_conidtional_distribution_bn(self, table_queries, join_keys, equivalent_group):
        ...
        for table in join_keys:
            ...
            res[table] = Factor(variables=id_attrs, pdfs=probs, equivalent_variables=new_id_attrs,
                                table_len=1.0, na_values={key: 1.0 for key in id_attrs})
        return res
    ...
    def get_cardinality_bound_all(self, query_str, sub_plan_query_str_all, query_name=None, debug=False,
        ...
        """
        tables_all, table_queries, join_cond, join_keys = self.parse_query_simple(query_str)
        for table in list(tables_all.values()):
            tables_all.setdefault(table, table)
        if isinstance(join_cond, list):
            join_cond_by_table = dict()
            for cond in join_cond:
                _, _, join, join_key = process_condition(cond)
                if join:
                    for tab in join_key:
                        if tab not in join_cond_by_table:
                            join_cond_by_table[tab] = set()
                        join_cond_by_table[tab].add(cond)
            join_cond = join_cond_by_table
        ...
        for group in union_key_group:
            ...
            for table, keys in union_key_group[group]:
                for key in keys:
                    ...
                    if table == "left":
                        key_group_pdf[key] = self._factor_key_pdf(cond_factor_left, key)
                        key_group_bin_mode[key] = self.table_buckets[tables_all[left_table]].oned_bin_modes[key]
                        new_na_values[key] = self._factor_key_na(cond_factor_left, key)
                    else:
    ...
    def join_two_tables(self, sub_plan_query_str, left_table, right_table, tables_all, conditional_factors, join_keys,
        ...
        for key_group in equivalent_key_group:
            ...
            all_pdfs = [self._factor_key_pdf(cond_factor_left, key) * cond_factor_left.table_len * self._factor_key_na(cond_factor_left, key)
                       ...
                       [self._factor_key_pdf(cond_factor_right, key) * res * self._factor_key_na(cond_factor_right, key)
        ...
        for group in union_key_group:
            ...
            for table, keys in union_key_group[group]:
                for key in keys:
                    new_union_key_group[group].append(key)
                    key_group_pdf[key] = self._factor_key_pdf(conditional_factors[table], key)
                    key_group_bin_mode[key] = self.table_buckets[tables_all[table]].oned_bin_modes[key]
                    new_na_values[key] = self._factor_key_na(conditional_factors[table], key)
