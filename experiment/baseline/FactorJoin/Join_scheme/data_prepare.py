    ...
    for table_obj in schema.tables:
        ...
        for attr in df_rows.columns:
            if attr in all_keys:
                ...
                if pd.api.types.is_numeric_dtype(df_rows[attr]):
                    temp = df_rows[attr].values
                    null_values[table_name][attr] = np.nanmin(temp) - 100 if len(temp) else -100
                    temp[pd.isna(temp)] = null_values[table_name][attr]
                else:
                    null_values[table_name][attr] = "__NULL__"
                    df_rows[attr] = df_rows[attr].fillna(null_values[table_name][attr])
