...
pub struct GraphContainer {
    ...
    /// GCard: standalone read-optimised graph for sampling (type-erased FlatGraph;
...
impl GraphContainer {
    ...
    /// GCard: store a pre-built FlatGraph for DB-free Wander Join sampling.
    pub fn set_gcard_flat_graph(&self, v: Arc<dyn Any + Send + Sync>) {
        *self.gcard_flat_graph.write().expect("RwLock write") = Some(v);
    }

    /// GCard: get the FlatGraph (downcast to `FlatGraph` in `minigu-core`).
    pub fn gcard_flat_graph(&self) -> Option<Arc<dyn Any + Send + Sync>> {
